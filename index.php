...
content: "holding..."
	...
	getOnibus();
	 ...
	 if(name != ""){
		...
		'address': name + ', São Paulo, SP', 'country': 'BR',  }, function (results, status) {
	...
	$(document).ready(function () {
		...
		source: function (request, response) {
			geocoder = new google.maps.Geocoder();
			geocoder.geocode({ 'address': request.term + ', São Paulo - SP', 'region': 'BR' }, function (results, status) {
		...
		success: function(dados){
			...
			for(var i=0;dados.length>i;i++){
				...
				var location = new google.maps.LatLng(dados[i].stop_lat, dados[i].stop_lon);
				 ...
				 var markerbusca = new google.maps.Marker({ map: map, icon: image, html:nome});									
					...
					marker1 = new google.maps.Marker({ map: map, icon: image, html:nome});									
	...
	$.ajax({
		...
		success: function(dados){
			...
			for(var i=0;dados.l.length>i;i++){
					var nome="";
					nome += ' <h4 style="color:#D7B610; margin:0; ">Ônibus </h4> <hr> <h5> Letreiro Completo:' + dados.l[i].c + '</h5> </br>';
					...
					if(dados.l[i].vs[j].py >= latmin && dados.l[i].vs[j].py <= latmax && dados.l[i].vs[j].px >= lngmin && dados.l[i].vs[j].px <= lngmax)
					{
					var	 veiculo;
					...
					marker1 = new google.maps.Marker({ map: map, icon: image, html:veiculo});									
  ...
  <input  id= "parada" type="checkbox" name="vehicle" value="parada"  onchange="getPO()" > Paradas de Ônibus
	...
	<div id="grupo">
		<h3> Integrantes </h3>
		<p> Ana Caroline Azevedo dos Reis 9423982 </p>
		<p> Luiz Carlos Marcelino Junior 9761635 </p>
		<p> Matheus Garcia Fagundes  9779030 </p>
		<p> Ygor Nanni de Carvalho 9778690 </p>
		</div>
